...
def play_game(env, models = {"black_0" : None,"white_0": None},episode_length = None,model_elo = 0, save_game_flag = True,render = False,eval = False,eps = None,num_sims = None, model_id = 0, cfg = cfg):
    ...
    for agent in env.agent_iter():
        ...
        if ply_count >= episode_length and reward == 0:
            if cfg.env_type == "go":
                try:
                    env.step(cfg.action_size[cfg.env_type]-1)
                    obs,reward,done,truncated,info = env.last()
                    if agent == cfg.agents[cfg.env_type][0]:
                        reward *= -1
                    if not done:
                        env.step(cfg.action_size[cfg.env_type]-1)
                        obs,reward,done,truncated,info = env.last()
        ...
        if render:
            ...
            plt.show()
    if (reward != 0.0 and cfg.env_type == "go" and save_game_flag) or save_game_flag:
        ...
        save_duration = perf_counter() - start_timer
...
def self_play(max_patience = 5,cfg = cfg):
    ...
    for i in range(int(cfg.max_n_episodes//cfg.episodes_per_epoch)):
        eps = eps_scheduler(i,cfg)
        learning_rate = lr_scheduler(i,cfg)
        ...
        if i % 40 == 0 and i > 0:
            eps = max(eps / 2,0.05)
            print(f"Epsilon updated to {eps}")
        ...
        if i % 5 == 0 and i > 0:
            ...
            # TODO: this elo diff needs to be updated to actual elo
